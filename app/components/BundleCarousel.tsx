...
import { BUNDLES, type BundleProduct } from "../store/bundles";
...
export default function BundleCarousel() {
  ...
  return (
    <section className="relative bg-gradient-to-b from-brand-50/50 to-white py-20 lg:py-28 overflow-hidden">
      {/* Background decoration */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-brand-100/30 via-transparent to-transparent" />
      ...
      <div className="relative mx-auto max-w-7xl px-5 lg:px-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <Reveal direction="up">
            <div>
              ...
              <p className="mt-3 max-w-xl text-sm text-navy-950/55 sm:text-base">
                Get more designs for less with our curated patch packs. Premium quality embroidery files at unbeatable prices — save up to 25%!
              </p>
            </div>
          </Reveal>
          ...
          <Reveal direction="up" delay={100} className="flex gap-3">
            <button
              ...
              className={`flex h-12 w-12 items-center justify-center rounded-full border-2 bg-white shadow-lg transition-all duration-300 ${
                canScrollLeft 
          ...
          >
            {BUNDLES.map((bundle, i) => (
              <Reveal key={bundle.slug} direction="up" delay={stagger(i, 80)}>
                <article
                  data-card
                  className="group relative flex w-[300px] flex-shrink-0 flex-col overflow-hidden rounded-2xl bg-white shadow-lg ring-1 ring-navy-950/8 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl snap-start sm:w-[320px] md:w-[340px]"
                >
                  ...
                  {/* Image Container - Large preview area */}
                  ...
                  <div className="flex flex-1 flex-col p-5 pb-4">
                    ...
                    <ul className="mt-3 space-y-1.5">
                      {bundle.features.slice(0, 3).map((feature, idx) => (
                        ...
                        >
                          <svg className="h-3.5 w-3. shrink-0 text-emerald-500" fill="currentColor" viewBox="0 0 20 20">
        ...
        <Reveal direction="up" className="mt-12 text-center">
          <Link
            href="/store"
            className="vr-btn vr-btn-primary inline-flex items-center gap-2 rounded-xl bg-brand-600 px-10 py-4.5 text-sm font-bold uppercase tracking-wide text-white shadow-xl shadow-brand-600/25 transition-all hover:bg-brand-700 hover:shadow-2xl hover:shadow-brand-600/30"
          >
            View All Bundles
            <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
